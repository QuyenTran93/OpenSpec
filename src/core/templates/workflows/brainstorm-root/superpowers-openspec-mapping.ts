...
1. \`openspec-brainstorm\` / \`/opsx:brainstorm\` — **superpowers:brainstorming** → \`brainstorm.md\` (self-review + user review per schema); readiness gate before **propose**.
...
3. \`openspec-writing-plans\` / \`/opsx:writing-plans\` — run **superpowers:writing-plans** (plan self-review + user review of \`plan.md\`, per \`schemas/brainstorm-root/schema.yaml\`); write \`openspec/changes/<change-name>/plan.md\` (required before apply), reading \`brainstorm.md\` and \`tasks.md\`.
4. \`openspec-apply-change\` / \`/opsx:apply\` — **default:** implement from \`plan.md\` **inline** in the primary session; track coarse progress in \`tasks.md\` checkboxes. Use subagents or the Task tool **only** for **review** or **read-only research** (per \`schemas/brainstorm-root/schema.yaml\` \`apply.instruction\`).
