...
1. \`openspec-brainstorm\` / \`/opsx:brainstorm\` — run **superpowers:brainstorming** interactively (including spec self-review and user review of \`brainstorm.md\`, per \`schemas/brainstorm-root/schema.yaml\`); capture the **final user-approved** outcome in \`openspec/changes/<change-name>/brainstorm.md\` only after readiness gate conditions are met.
